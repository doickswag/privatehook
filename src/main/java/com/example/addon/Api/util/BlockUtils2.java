...
public class BlockUtils2 {
    public static boolean placeBlock(FindItemResult result, BlockPos pos, boolean rotate, int rotationPriority, boolean airPlace, boolean ignoreEntity, boolean swing, boolean strictDirections) {
        if (!result.found()) {
            return false;
        } else if (invalidPos(pos)) {
            return false;
        } else if (!GANGTIL.replaceable(pos)) {
            return false;
        } else {
    ...
    public static void justPlace(FindItemResult item, BlockHitResult hitResult, boolean swing, boolean rotate, int rotationPriority) {
        if (rotate) {
            Rotations.rotate(Rotations.getYaw(hitResult.getPos()), Rotations.getPitch(hitResult.getPos()), rotationPriority, () -> justPlace(item, hitResult, swing, false, 0));
        } else {
            ...
            boolean sneak = !MeteorClient.mc.player.isSneaking() && clickableBlock(MeteorClient.mc.world.getBlockState(hitResult.getBlockPos()), hitResult);
            if (sneak) {
    ...
    public static boolean outOfPlaceRange(BlockPos pos, Origin origin, double range) {
        if (origin == Origin.VANILLA) {
            return MeteorClient.mc.player.squaredDistanceTo((double) pos.getX() + 0.5, (double) pos.getY() + 0.5, (double) pos.getZ() + 0.5) >= range * range;
        } else {
            Vec3d eyesPos = PlayerUtils2.eyePos(MeteorClient.mc.player);
            double dx = eyesPos.x - (double) pos.getX() - 0.5;
            double dy = eyesPos.y - (double) pos.getY() - 0.5;
    ...
    @Nullable
    public static BlockHitResult getPlaceResult(BlockPos block, boolean airPlace, boolean strictDirections, @Nullable ItemPlacementContext context, @Nullable BlockItem item) {
        BlockPos neighbor = null;
        Direction placeSide = null;
        if (strictDirections) {
            Pair<BlockPos, Direction> pair = getStrictNeighbour(block);
            if (pair != null) {
                neighbor = pair.getLeft();
                placeSide = pair.getRight();
            }
        } else {
        ...
        Vec3d eyesPos = PlayerUtils2.eyePos(MeteorClient.mc.player);
        Vec3d vec = new Vec3d(MathHelper.clamp(eyesPos.x, neighbor.getX(), neighbor.getX() + 1), MathHelper.clamp(eyesPos.y, neighbor.getY(), neighbor.getY() + 1), MathHelper.clamp(eyesPos.z, neighbor.getZ(), neighbor.getZ() + 1));
    ...
    public static Vec3d getCenterPos(boolean hardSnap) {
        ...
        return new Vec3d((double) MeteorClient.mc.player.getBlockX() + x, MeteorClient.mc.player.getY(), (double) MeteorClient.mc.player.getBlockZ() + z);
    }
    ...
    public static Set<BlockPos> getCity(LivingEntity player, boolean down, boolean check) {
        ...
        BlockPos minMin = playerBlockPos.add((int) (box.minX - (double) playerBlockPos.getX()), 0, (int) (box.minZ - (double) playerBlockPos.getZ()));
        ...
        BlockPos minMax = playerBlockPos.add((int) (box.minX - (double) playerBlockPos.getX()), 0, (int) (box.maxZ - (double) playerBlockPos.getZ()));
        ...
        BlockPos maxMin = playerBlockPos.add((int) (box.maxX - (double) playerBlockPos.getX()), 0, (int) (box.minZ - (double) playerBlockPos.getZ()));
        ...
        BlockPos maxMax = playerBlockPos.add((int) (box.maxX - (double) playerBlockPos.getX()), 0, (int) (box.maxZ - (double) playerBlockPos.getZ()));
    ...
    public static Pair<BlockPos, Direction> getStrictNeighbour(BlockPos pos) {
        ...
        for (Direction direction : Direction.values()) {
            ...
            if (!GANGTIL.replaceable(neighbour) || Statistics.get().pendingObsidian.containsKey(neighbour)) {
                Direction opposite = direction.getOpposite();
                if (AntiCheatHelper.isInteractableStrict(MeteorClient.mc.player.getBlockX(), playerEyeY, MeteorClient.mc.player.getBlockZ(), neighbour, opposite)) {
    ...
    public static Direction getStrictSide(BlockPos pos) {
        ...
        for (Direction direction : Direction.values()) {
            if (AntiCheatHelper.isInteractableStrict(MeteorClient.mc.player.getBlockX(), playerEyeY, MeteorClient.mc.player.getBlockZ(), pos, direction)) {
    ...
    public static Vec3d sideVec(BlockPos pos, Direction direction) {
        return Vec3d.ofCenter(pos).add((double) direction.getOffsetX() * 0.5, (double) direction.getOffsetY() * 0.5, (double) direction.getOffsetZ() * 0.5);
    }
    ...
    public static Direction getClosestDirection(BlockPos pos, boolean withSupport) {
        ...
        for (Direction direction : Direction.values()) {
            BlockPos neighbour = pos.offset(direction);
            if (!withSupport || !GANGTIL.replaceable(neighbour) || Statistics.get().pendingObsidian.containsKey(neighbour)) {
                if (direction == Direction.DOWN) {
                    return direction;
                }
    ...
    public static boolean noSupport(BlockPos pos) {
        for (Direction side : Direction.values()) {
            ...
            if (!GANGTIL.replaceable(neighbor)) {
                return false;
            }
        }
    ...
    public static boolean clickableBlock(BlockState state, BlockHitResult hit) {
        return state.onUse(MeteorClient.mc.world, MeteorClient.mc.player, hit) != ActionResult.PASS;
    }
}
