...
import com.example.addon.Api.Elite.PlayerListChangeEvent;
...
public class Statistics {
        ...
        if (entity instanceof EntityStatusS2CPacket packet) {
            ...
            if (entity instanceof BlockUpdateS2CPacket packet) {
                ...
                if (entity instanceof EntityPositionS2CPacket packet) {
                    this.accuratePos.put(packet.getEntityId(), new Vec3d(packet.getX(), packet.getY(), packet.getZ()));
                }
            }
        }
    }
    ...
    private void onTick(Post event) {
        if (MeteorClient.mc.player != null && MeteorClient.mc.world != null) {
            int latency = 100;
            PlayerListEntry networkEntry = MeteorClient.mc.player.networkHandler.getPlayerListEntry(MeteorClient.mc.getSession().getUuidOrNull());
    ...
    private void onMessageRecieve(Receive event) {
        ...
        if (msg instanceof GameMessageS2CPacket packet && packet.content() != null) {
            ...
            if (byIndex != -1) {
                int nameIndex = msgx.indexOf(MeteorClient.mc.player.getName().getString());
                if (nameIndex >= byIndex) {
                    String victimsName = msgx.substring(0, msgx.indexOf(" "));

                    for (AbstractClientPlayerEntity player : MeteorClient.mc.world.getPlayers()) {
                        if (player.getName().getString().equals(victimsName) && this.targets.contains(player)) {
                            return;
                        }
                    }
                    ...
                    for (final PlayerListEntry target : MeteorClient.mc.getNetworkHandler().getPlayerList()) {
                        if (target.getProfile().getName().equals(victimsName)) {
                            victim = (PlayerEntity)new OtherClientPlayerEntity(MeteorClient.mc.world, new GameProfile(target.getProfile().getId(), victimsName));
                            break;
                        }
                    }
    ...
    public boolean isTarget(PlayerListEntry playerListEntry) {
        for (PlayerEntity player : this.targets) {
            if (player.getName().getString().equals(playerListEntry.getProfile().getName())) {
                return true;
            }
        }
