...
import com.example.addon.Api.Elite.IBlockItem;
...
public class PlayerUtils2 {
    ...
    public static Vec3d predictPos(PlayerEntity player, int ticks, int antiStepOffset) {
        ...
        if (UtilsPlus.isSurrounded(player, true, true) && v.y <= 0.0) {
            ...
            for (int i = 0; i <= ticks; ++i) {
                BlockPos newPos = new Mutable(nextPos.add(v).x, nextPos.add(v).y, nextPos.add(v).z);
                if (collisionCheck(newPos) || Math.ceil(player.getEyeHeight(player.getPose())) != 1.0 && collisionCheck(newPos.up())) {
                    if (antiStepOffset >= 1 && !hasStepped && collisionCheck(newPos) && !collisionCheck(newPos.up()) && !collisionCheck(newPos.up(2))) {
                        nextPos = nextPos.add(v).add(0.0, 1.0, 0.0);
                        hasStepped = true;
                    } else if (antiStepOffset >= 2 && !hasStepped && collisionCheck(newPos.up()) && !collisionCheck(newPos.up(2)) && !collisionCheck(newPos.up(3))) {
    ...
    public static Box predictBox(PlayerEntity player, int ticks, int antiStepOffset) {
        ...
        double dx = oldBox.getLengthX() * 0.5;
        double dy = oldBox.getLengthY();
        double dz = oldBox.getLengthZ() * 0.5;
        return new Box(nextPos.x - dx, nextPos.y, nextPos.z - dz, nextPos.x + dx, nextPos.y + dy, nextPos.z + dz);
    }
    ...
    private static boolean collisionCheck(BlockPos pos) {
        if (collisions.containsKey(pos)) {
            ...
            boolean collides = UtilsPlus.isCollides(pos);
    ...
    public static boolean canPlace(ItemPlacementContext context, BlockItem blockItem) {
        if (!context.canPlace()) {
            ...
            if (itemPlacementContext == null) {
                ...
                return blockState != null && ((IBlockItem) blockItem).canBePlaced(itemPlacementContext, blockState);
            }
        }
    }

    public static void collectTargets(List<PlayerEntity> targets, @Nullable List<PlayerEntity> friends, int targetRange, int maxTargets, boolean ignoreNakeds, boolean onlyHoled, boolean ignoreTerrain, SortPriority sortPriority) {
        ArrayList<LivingEntity> livings = new ArrayList();
        collectTargets(livings, friends, targetRange, maxTargets, ignoreNakeds, onlyHoled, ignoreTerrain, sortPriority, UtilsPlus.asSet(EntityType.PLAYER));
        targets.clear();
    ...
    public static void collectTargets(List<LivingEntity> targets, @Nullable List<PlayerEntity> friends, int targetRange, int maxTargets, boolean ignoreNakeds, boolean onlyHoled, boolean ignoreTerrain, SortPriority sortPriority, Set<EntityType<?>> entities) {
        targets.clear();
        if (friends != null) {
            friends.clear();
        }
        ...
        for (Entity entity : MeteorClient.mc.world.getEntities()) {
            if (entity instanceof LivingEntity living && !living.isDead() && entity != MeteorClient.mc.player && !(MeteorClient.mc.player.distanceTo(entity) > (float) targetRange)) {
                if (entity instanceof PlayerEntity player) {
                    if (EntityUtils.getGameMode(player) != GameMode.CREATIVE) {
                        if (Friends.get().shouldAttack(player)) {
                            if (entities.contains(EntityType.PLAYER) && (!ignoreNakeds || !UtilsPlus.isNaked(player)) && (!onlyHoled || UtilsPlus.isSurrounded(player, true, ignoreTerrain))) {
                                targets.add(player);
                            }
                ...
                } else if (entities.contains(entity.getType())) {
                    targets.add(living);
                }
            }
        }
