...
import com.example.addon.Api.util.holes.enums.SurroundBlocks;
...
public class InstantSelfFill extends Hooked {
    ...
    private final StaticListener BURROW_LISTENER = new StaticListener();
    public final Setting<Boolean> autoReburrow = this.setting("auto-burrow-replenish", "Automatically burrows you again when someone mines your burrow block.", Boolean.valueOf(false), this.sgAutomation);
    private Vec3d playerPos;
    private FindItemResult result;
    public final Setting<Boolean> pauseEating = this.setting("pause-while-eating", "Will not automatically burrow you again when you are eating.", Boolean.valueOf(true), this.sgAutomation, () -> this.autoReburrow.get() || this.autoTrap.get());
    ...
    public InstantSelfFill() {
        super(Hook.CATEGORY, "InstantSelfFill", "Attempts to place a block inside of your feet.");
    ...
    public void onActivate() {
        ...
        if (!this.mc.player.isOnGround()) {
            this.toggleWithInfo(Text.translatable("burrow.ground").toString());
        } else {
            FindItemResult result = this.block.get().getBlock();
            if ((result.isHotbar() || result.isOffhand()) && result.found()) {
                ...
                Mutable playerBlock = new Mutable(this.playerPos.x, onEchest ? Math.ceil(this.playerPos.y) : this.playerPos.y, this.playerPos.z);
                if (BlockUtils2.invalidPos(playerBlock)) {
                    this.toggleWithInfo(Text.translatable("burrow.world").toString());
                } else {
                    BlockHitResult hitResult = BlockUtils2.getPlaceResult(playerBlock, this.airPlace.get(), this.strictDirections.get());
                    if (hitResult == null) {
                        this.toggleWithInfo(Text.translatable("burrow.place").toString());
                    } else {
                        float eyeHeight = this.mc.player.getEyeHeight(this.mc.player.getPose());
                        if (!this.phoenixMode.get() && this.collides(Math.ceil(eyeHeight))) {
                            this.toggleWithInfo(Text.translatable("burrow.headroom").toString());
                        } else {
                            BlockState state = this.mc.world.getBlockState(playerBlock);
                            if (!GANGTIL.replaceable(playerBlock) && (!(state.getBlock() instanceof BedBlock) || this.mc.world.getRegistryKey() == World.OVERWORLD) && state.getBlock().getHardness() > 0.0F && this.collides(onEchest ? 1.0 : 0.0)) {
                                this.toggleWithInfo(Text.translatable("burrow.burrowed").toString());
                            } else {
                                VoxelShape placeShape = ((BlockItem) PlayerUtils2.getItemFromResult(result)).getBlock().getDefaultState().getCollisionShape(this.mc.world, playerBlock);
                                ...
                                if (entity == null || this.attackCrystals.get() && entity instanceof EndCrystalEntity) {
                                    ...
                                    for (int rubberHeight = this.minRubberbandHeight.get(); rubberHeight <= this.maxRubberbandHeight.get(); ++rubberHeight) {
                                        if ((rubberHeight < this.minBlacklist.get() || rubberHeight > this.maxBlacklist.get()) && (rubberHeight <= -3 || rubberHeight >= 2) && !this.collides(rubberHeight)) {
                                            if (!this.collides((double) rubberHeight + Math.floor(eyeHeight))) {
                                                if (this.phoenixMode.get() && this.collides(Math.ceil(eyeHeight))) {
                                                    boolean cantJump = true;

                                                    for (double i = -4.0; i < 5.0; ++i) {
                                                        if (i != 0.0 && i != -1.0 && !this.collides(i)) {
                                                            if (this.collides(i + Math.floor(eyeHeight))) {
                                                                if (eyeHeight > 1.0F) {
                                                                    ++i;
                                                                }
                                                            } else {
                                                                ...
                                                                if ((double) rubberHeight - i < 5.0 && i - (double) rubberHeight < 9.0) {
                                                                    ...
                                                                    for (int j = rubberHeight; j <= this.maxRubberbandHeight.get(); ++j) {
                                                                        if ((j < this.minBlacklist.get() || j > this.maxBlacklist.get()) && (j <= -3 || j >= 3) && (!((double) j - i < 5.0) || !(i - (double) j < 9.0)) && !this.collides(j)) {
                                                                            if (!this.collides((double) j + Math.floor(eyeHeight))) {
                                                                                rubberHeight = j;
                                                    ...
                                                    if (onEchest) {
                                                        double maxY = placeShape.offset(playerBlock.getX(), playerBlock.getY(), playerBlock.getZ()).getMax(Axis.Y);
                                    ...
                                    this.toggleWithInfo(Text.translatable("burrow.rubberband").toString());
                                } else {
                                    this.toggleWithInfo(Text.translatable("burrow.entities").toString());
                                }
                            }
                        }
                    }
                ...
                this.toggleWithInfo(Text.translatable("burrow.block").toString());
            }
        }
    }
    ...
    private void onLateTick(Pre event) {
        if (this.rotate.get() && this.playerBlock != null) {
            this.mc.player.networkHandler.sendPacket(new LookAndOnGround(this.mc.player.getYaw(), 90.0F, this.mc.player.isOnGround()));
    ...
    private void performBurrow(FindItemResult result, BlockHitResult hitResult, BlockPos playerBlock, BlockState state, @Nullable Entity entity) {
        ...
        for (int i = 0; i < this.packetList.size(); ++i) {
            ...
            if (i < this.packetList.size() - 1) {
                this.mc.player.networkHandler.sendPacket(new PositionAndOnGround(this.mc.player.getX(), this.mc.player.getY() + height, this.mc.player.getZ(), true));
            } else {
                if (entity != null) {
                ...
                this.mc.player.networkHandler.sendPacket(new PositionAndOnGround(this.mc.player.getX(), this.mc.player.getY() + height, this.mc.player.getZ(), false));
                this.toggle();
            }
        }
    }
    ...
    private class StaticListener {
        ...
        private void surroundListener(Post event) {
            --this.delay;
            if (UtilsPlus.isObbyBurrowed(InstantSelfFill.this.mc.player) && InstantSelfFill.this.mc.world.getBlockState(InstantSelfFill.this.mc.player.getBlockPos()).getBlock().getBlastResistance() > 600.0F) {
                this.pos = InstantSelfFill.this.mc.player.getBlockPos();
            } else if (!InstantSelfFill.this.isActive() && InstantSelfFill.this.mc.player.isOnGround() && !UtilsPlus.isBurrowed(InstantSelfFill.this.mc.player) && (!InstantSelfFill.this.pauseEating.get() || !InstantSelfFill.this.mc.player.isUsingItem())) {
                if (InstantSelfFill.this.autoReburrow.get() && InstantSelfFill.this.mc.player.getBlockPos().equals(this.pos)) {
                    this.pos = null;
                    InstantSelfFill.this.toggle();
                } else {
                    if (this.delay <= 0 && InstantSelfFill.this.autoTrap.get() && UtilsPlus.isSurrounded(InstantSelfFill.this.mc.player, false, false)) {
                        for (PlayerEntity enemy : InstantSelfFill.this.mc.world.getPlayers()) {
                            if (!InstantSelfFill.this.mc.player.equals(enemy) && !(InstantSelfFill.this.mc.player.distanceTo(enemy) > 5.0F)) {
                                if (InstantSelfFill.this.mc.player.getBlockPos().equals(enemy.getBlockPos())) {
                                    break;
                                }

                                if (Friends.get().shouldAttack(enemy) && !UtilsPlus.isSurrounded(enemy, true, true) && InstantSelfFill.this.mc.player.getPos().add(0.0, 1.0, 0.0).distanceTo(enemy.getPos()) <= 2.0 && enemy.getY() > InstantSelfFill.this.mc.player.getY()) {
                                    InstantSelfFill.this.toggle();
                                    this.delay = 20;
                                    return;
                                }
