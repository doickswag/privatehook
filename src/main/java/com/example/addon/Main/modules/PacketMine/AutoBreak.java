...
public class AutoBreak extends Hooked {
    ...
    private final Setting<Boolean> mineair = this.setting("MineAir", "continues to mine air", Boolean.valueOf(false));
    ...
    private final Setting<Boolean> eatPause = this.setting("pause-while-eating", "Won't swap slots while you are eating, to not interupt it.", Boolean.valueOf(true), this.sgGeneral, () -> this.autoMine.get() && !this.strict.get());
    private final Setting<Integer> maxInstaMineAttempts = this.setting("insta-mine-attempts", "How many times you want to attempt to insta mine in a row without having to remine.", Integer.valueOf(1), this.sgGeneral, this.autoMine::get);
    ...
    private final Setting<Boolean> percColors = this.setting("progress-colors", "Will render red when starting and green when finished with gradient.", Boolean.valueOf(false), this.sgGeneral, this.render::get);
            ...
            if (outOfMiningRange(this.pos, this.origin.get(), this.range.get())) {
                ...
                if (this.progress >= 1.0F) {
                    ...
                    if (!this.hasSwitched && this.amountOfInstaBreaks <= this.maxInstaMineAttempts.get()) {
                        ...
                        PlayerListEntry playerlistEntry = this.mc.player.networkHandler.getPlayerListEntry(mc.player.getName().getString());
    ...
    private double getBreakDelta(int slot, BlockState state) {
        ...
        if (playerlistEntry != null && playerlistEntry.getGameMode() == GameMode.CREATIVE) {
            ...
            if (hardness == -1.0F) {
                ...
                float speed = this.mc.player.getInventory().main.get(slot).getMiningSpeedMultiplier(state);
                if (speed > 1.0F) {
                ...
                if (this.mc.player.isSubmergedIn(FluidTags.WATER) && !GANGTIL.hasAquaAffinity(this.mc.player)) {
                    speed /= 5.0F;
                }
                ...
                return speed / hardness / (float) (state.isToolRequired() && !this.mc.player.getInventory().main.get(slot).isSuitableFor(state) ? 100 : 30);
            }
        }
    }
