...
public class DiscordRPC extends Hooked {
    ...
    // Line 1
    private final Setting<Mode> type = sgGeneral.add(new EnumSetting.Builder<Mode>().name("type").defaultValue(Mode.hook).build());
    ...
    // Line 2
    private final Setting<List<String>> line2Strings = sgLine2.add(new StringListSetting.Builder().name("line-2-messages").description("Messages used for the second line.").defaultValue("PrivateHook >","Owning {server}.", "spawn was cleared!", "GG predicted by PrivateHook").onChanged(strings -> recompileLine2()).build());
    ...
    public DiscordRPC() {
        super(Hook.CATEGORY, "DiscordRPC", "Displays a presence on Discord.");
    ...
    public void onActivate() {
        String id = "hook", image = "hook", name = "hook";
        switch (type.get()) {
            case hook -> {
                id = "1389450996425429104";
                image = "1";
                name = " ";
            }
            case BedTrap -> {
        ...
        rpc.setLargeImage(image, BuildInfo.COMMIT);
