...
public class TNTAura extends Hooked {
    ...
    public TNTAura() {
        super(Hook.CATEGORY, "TNTAura", "Blows the opps up with the t-a-tnt");
    }
        ...
        if (target != null) {
            if (!obsidianPos.isEmpty()) {
                for (BlockPos pos : obsidianPos) {
                    Vector3d targetPos3D = new Vector3d(pos.getX(), pos.getY(), pos.getZ());
                    if (NametagUtils.to2D(targetPos3D, scale.get())) {
                        String targetName = String.valueOf(target.getName().getString());
                        NametagUtils.begin(targetPos3D);
                        TextRenderer.get().begin(1.0, false, true);
                        TextRenderer.get().render(targetName, -TextRenderer.get().getWidth(targetName) / 2.0, 0.0, new Color(255, 0, 0, 255));
