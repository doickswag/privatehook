...
import com.example.addon.Api.Elite.EnchantmentSelectScreen;
...
public class VillagerRoller extends Module {
    ...
    private final Setting<Boolean> Sysem = sgGeneral.add(new BoolSetting.Builder()
        .name("System Notification")
        .description("send notif")
        .defaultValue(true)
        .build()
    );
    ...
    public VillagerRoller() {
        super(Hook.CATEGORY, "villager-roller", "Rolls trades.");
    }
            ...
            for (Pair<RegistryEntry<Enchantment>, Integer> enchant : getEnchants(sellItem)) {
                ...
                for (RollingEnchantment e : searchingEnchants) {
                    ...
                    if (disconnectIfFound.get()) {
                        ...
                        if (Sysem.get()) {
                            SystemNotification.send("PrivateHook", message);
                        }
                    }
                    break;
                }
    ...
    private void onInteractEntity(InteractEntityEvent event) {
        ...
        if (Sysem.get()) {
            SystemNotification.send("PrivateHook", "We got your villager!");
        }
        event.cancel();
    }

    @EventHandler(priority = EventPriority.HIGH)
    private void onStartBreakingBlockEvent(StartBreakingBlockEvent event) throws AWTException {
